...
class ICTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        batch_orig = batch["orig"]
        batch_adv = batch["adv"]
        
        # original loss
        token_ids_orig, attention_mask_orig = batch_orig['supBatch']['token_ids'], batch_orig['supBatch']['mask']
        intent_target_orig = batch_orig['supBatch']['intent_id']
        out_orig = self(token_ids_orig,attention_mask_orig,intent_target_orig)

        # adversarial examples loss
        token_ids_adv, attention_mask_adv = batch_adv['supBatch']['token_ids'], batch_adv['supBatch']['mask']
        intent_target_adv = batch_adv['supBatch']['intent_id']
        out_adv = self(token_ids_adv,attention_mask_adv,intent_target_adv)
        
        # logging
        self.log('train_IC_loss', out_orig['ic_loss'] + out_adv['ic_loss'], on_step=False, on_epoch=True, logger=True)
        
        return out_orig['ic_loss'] + out_adv['ic_loss']
